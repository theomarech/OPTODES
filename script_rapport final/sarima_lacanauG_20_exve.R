...
rmse_base <- RMSE(sarima_base_pred$pred %>% as.vector(),df_lag_log$conc_od[-indice])
mae_base <- MAE(sarima_base_pred$pred %>% as.vector(),df_lag_log$conc_od[-indice])
sse_base <- var(df_lag_log$conc_od[-indice]-sarima_base_pred$pred %>% as.vector())
############################## sans température ###############################
...
rmse_st <- RMSE(sarima_pred_2bev$pred %>% as.vector(),df_test_base$conc_od)
mae_st <- MAE(sarima_pred_2bev$pred %>% as.vector(),df_test_base$conc_od)
...
df_train_temp <- df_lag_log_temp[indice,] %>% dplyr::select(-date)
df_test_temp <- df_lag_log_temp[-indice,]%>% dplyr::select(-date)
...
data_lacanauG_exve_20_sarima_pred_1 <- bind_cols(
